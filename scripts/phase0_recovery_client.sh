...
OUT_BASE="${OUT_BASE:-logs/RecoveryGen/Phase0/client_logs/${RUN_TAG}}"
mkdir -p "${OUT_BASE}"
...
if [[ "${OUT_BASE}" == "logs/RecoveryGen/Phase0/client_logs/${RUN_TAG}" ]]; then
  OUT_BASE="logs/RecoveryGen/Phase0/client_logs/${RUN_TAG}_${SERVER_SIG_STR}_${GATE_SIG}"
  mkdir -p "${OUT_BASE}"
fi
...
for ((c=1; c<=NUM_CYCLES; c++)); do
  ...
  run_phase "${c}" "low" "${LOW_LAMBDA}" "$((SEED_ARRIVAL_BASE + c*2))" "${MODE_LOW}" "${LOW_S}" || break

  log_phase_change "${c}" "high" "${HIGH_LAMBDA}" "${MODE_HIGH}"
  run_phase "${c}" "high" "${HIGH_LAMBDA}" "$((SEED_ARRIVAL_BASE + c*2 + 1))" "${MODE_HIGH}" "${HIGH_S}" || break
done
rc_all=$?
set -e
