...
BASE_DIR="${BASE_DIR:-logs/RecoveryGen/Phase0}"
...
export VLLM_RECOVERY_OBS="${VLLM_RECOVERY_OBS:-0}"
...
TAG="phase0_cfg=${CFG_TAG}_served=${SERVED}_mem=${MEM}_maxlen=${MAXLEN}_mbt=${MAX_BATCH_TOKENS}_mseq=${MAX_NUM_SEQS}_pmode=${PMODE}_swap=${SWAP_SPACE_GB}_chunk=${ENABLE_CHUNKED_PREFILL}_ss=${NUM_SCHED_STEPS}_cvis=${CUDA_VISIBLE_DEVICES}_port=${PORT}"
